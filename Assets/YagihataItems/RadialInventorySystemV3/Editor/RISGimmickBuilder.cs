...
namespace YagihataItems.RadialInventorySystemV3
{
    public static class RISGimmickBuilder
    {
        ...
        public static void ApplyToAvatar(RISVariables variables)
        {
            ...
            if (variables.UseCommonMenu && (variables.CommonFXLayer == null || variables.CommonMenu == null || variables.CommonParameters == null))
            {
                EditorUtility.DisplayDialog("Radial Inventory System", "共通メニュー用のController, Expression Menu, Expression Parametersをセットして下さい。", "OK");
                return;
            }
        ...
        private static void BuildFXLayer(RISVariables variables, string autoGeneratedFolder)
        {
            ...
            // 共通メニューが有効のときコピーしてアタッチする
            AnimatorController fxLayer;
            if (variables.UseCommonMenu)
            {
                var path = autoGeneratedFolder + "GeneratedFXLayer.controller";
                var asset = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(path);
                if (asset != null)
                    AssetDatabase.DeleteAsset(path);
                AssetDatabase.CopyAsset(AssetDatabase.GetAssetPath(variables.CommonFXLayer), path);
                fxLayer = AssetDatabase.LoadAssetAtPath<AnimatorController>(path);
                if (avatar.baseAnimationLayers == null || avatar.baseAnimationLayers.Length < 5)
                {
                    avatar.baseAnimationLayers = new VRCAvatarDescriptor.CustomAnimLayer[]
                    {
                        ...
                        new VRCAvatarDescriptor.CustomAnimLayer(),
                        new VRCAvatarDescriptor.CustomAnimLayer(),
                        new VRCAvatarDescriptor.CustomAnimLayer(){ isEnabled = true, animatorController = fxLayer, type = VRCAvatarDescriptor.AnimLayerType.FX }
                    };
                }
                else
                {
                    avatar.baseAnimationLayers[4] = new VRCAvatarDescriptor.CustomAnimLayer() { isEnabled = true, animatorController = fxLayer, type = VRCAvatarDescriptor.AnimLayerType.FX };
                }
            }
            else
            {
                fxLayer = avatar.GetFXLayer(autoGeneratedFolder);
            }
        ...
        private static void BuildExpressionParameters(RISVariables variables, string autoGeneratedFolder)
        {
            ...
            // 共通メニューが有効のときコピーしてアタッチする
            VRCExpressionParameters expParams;
            if (variables.UseCommonMenu)
            {
                var path = autoGeneratedFolder + "GeneratedExpressionParameters.asset";
                var asset = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(path);
                if (asset != null)
                    AssetDatabase.DeleteAsset(path);
                AssetDatabase.CopyAsset(AssetDatabase.GetAssetPath(variables.CommonParameters), path);
                expParams = AssetDatabase.LoadAssetAtPath<VRCExpressionParameters>(path);
                avatar.expressionParameters = expParams;
            }
            else
            {
                expParams = avatar.GetExpressionParameters(autoGeneratedFolder);
            }
        ...
        private static void BuildExpressionsMenu(RISVariables variables, string autoGeneratedFolder)
        {
            ...
            if (variables.UseCommonMenu)
            {
                var path = autoGeneratedFolder + "AutoGeneratedMenu.asset";
                var asset = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(path);
                if (asset != null)
                    AssetDatabase.DeleteAsset(path);
                AssetDatabase.CopyAsset(AssetDatabase.GetAssetPath(variables.CommonMenu), path);
                rootMenu = AssetDatabase.LoadAssetAtPath<VRCExpressionsMenu>(path);
                avatar.expressionsMenu = rootMenu;
            }
            else
            {
                rootMenu = avatar.expressionsMenu;
                if (rootMenu == null)
